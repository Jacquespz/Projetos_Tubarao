    ...
    if ($action == 'envio') {
        ...
        foreach ($bercos as $berco_id) {
            ...
            if ($berco && $berco['filial_id'] == $filial_origem_id) {
                ...
                $stmt = $conn->prepare("INSERT INTO movimentacoes (berco_id, filial_origem_id, filial_destino_id, placa_caminhao, nome_motorista, usuario_id) VALUES (?, ?, ?, ?, ?, ?)");
                ...
                if ($stmt->execute()) {
                    ...
                    error_log("Erro ao registrar movimentação: " . $stmt->error);
                }
            } else {
                echo "O berço {$berco_id} não está na filial de origem especificada.";
                error_log("O berço {$berco_id} não está na filial de origem especificada.");
            }
        }
        ...
        foreach ($movimentacao_ids as $movimentacao_id) {
            ...
            if (!$stmt->execute()) {
                ...
                error_log("Erro ao inserir minuta: " . $stmt->error);
            }
        }
        ...
        foreach ($bercos as $berco_id) {
            ...
            if ($berco) {
                ...
                $stmt = $conn->prepare("INSERT INTO movimentacoes (berco_id, filial_origem_id, filial_destino_id, placa_caminhao, nome_motorista, usuario_id) VALUES (?, NULL, ?, ?, ?, ?)");
                ...
                if ($stmt->execute()) {
                    ...
                    if ($update_stmt->execute()) {
                        ...
                        error_log("Erro ao atualizar berço: " . $update_stmt->error);
                    }
                } else {
                    echo "Erro ao registrar movimentação: " . $stmt->error;
                    error_log("Erro ao registrar movimentação: " . $stmt->error);
                ...
                error_log("O berço {$berco_id} não está em trânsito.");
            }
        }
